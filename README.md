...
| 情况 | 行为 |
|---|---|
| 期权/期货合约行整类缺失,或期权条数少于基准快照**且空头总张数同步缩水**,或与官网敞口表差 >0.3 个百分点,或有腿被丢掉 | 判为**数据不全**;若「等一等可能会好」(分阶段发布)→ 不发信、**不写幂等标记**,等下次触发(10 分钟后再看) |
| 期权条数少于基准但**空头总张数持平**(如周度到期滚入月度,2026-08-24 即是)| 判为换仓而非缺数据:照常发信,黄色告警条留痕 |
